    ...
    <div className="flex flex-col h-full overflow-hidden">
      <header className="bg-[#1a1a2e] text-white shadow-lg shrink-0">
        <div className="px-6 py-5 flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold">Financeiro <span className="text-[#d4a853]">de Grupos</span></h1>
            <p className="text-sm text-gray-300 mt-1">Gestão financeira dos produtos de grupo</p>
          </div>
      ...
      <main className="flex-1 overflow-y-auto p-6">
        {filtered.length === 0 ? (
          <div className="text-center py-20">
            ...
            {grupos.length === 0 && (
              <Link href="/grupos">
                <Button className="mt-4 bg-[#d4a853] hover:bg-[#c49943] text-[#1a1a2e]">
                  Ir para Produtos
                </Button>
              </Link>
            )}
                    ...
                    <div className="flex items-start justify-between">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex gap-6 text-sm text-gray-500">
                          <span>Custo/pax: <strong className="text-gray-700">{formatBRL(summary.custoTotal)}</strong></span>
                          <span>Venda/pax: <strong className="text-[#d4a853]">{formatBRL(summary.precoVenda)}</strong></span>
                          <span>Vendas: <strong>{summary.vendas}</strong></span>
                          <span>Recebido: <strong>{summary.recebimentos}/{summary.totalParcelas}</strong></span>
                          <span className="text-xs text-gray-400">Atualizado: {formatDate(g.updated_at?.split('T')[0])}</span>
                        </div>
                      </div>
                        ...
                        {FIN_TABS.map(({ tab, label, icon: Icon }) => (
                          <Link key={tab} href={`/financeiro/${g.id}/${tab}`}>
                            <Button
                              variant="outline"
                              size="sm"
                              className="h-8 px-2 text-xs hover:bg-[#d4a853] hover:text-[#1a1a2e] hover:border-[#d4a853]"
                              title={label}
                            >
