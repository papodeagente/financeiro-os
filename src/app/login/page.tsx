...
export default function LoginPage() {
  ...
  const [campanha, setCampanha] = useState<CampanhaLogin | null>(null);

  // Carrega imagem de campanha editavel pelo super admin em
  // /admin/marketing. Se nao houver image_url ou campanha estiver
  // desativada, o painel esquerdo fica oculto (form ocupa tela toda).
  useEffect(() => {
    fetch('/api/marketing/login')
      .then(r => r.json())
      .then(data => {
        if (data && data.image_url && data.ativo !== false) {
          setCampanha(data as CampanhaLogin);
        }
      })
      .catch(() => { /* ignore — login funciona sem campanha */ });
  }, []);
  ...
  const hasCampanha = !!(campanha?.image_url);

  return (
    <div className={`min-h-screen flex flex-col ${hasCampanha ? 'lg:flex-row' : ''}`}>
      {/* Painel esquerdo — imagem de campanha editavel + link.
          So renderiza quando ha image_url configurada e ativa.
          Em mobile fica oculto (form ocupa tela toda). */}
      {hasCampanha && (
        <aside className="hidden lg:flex lg:w-[55%] xl:w-[60%] bg-[var(--t-bg)] items-center justify-center p-6 overflow-hidden">
          {campanha!.link_url ? (
            <a
              href={campanha!.link_url}
              target="_blank"
            ...
            >
              <img
                src={campanha!.image_url}
                alt={campanha!.alt}
                className="w-full h-full object-cover rounded-2xl shadow-2xl transition-transform group-hover:scale-[1.01]"
              />
            </a>
          ) : (
            <img
      ...
      {/* Painel direito (ou unico) — form ORIGINAL preservado */}
      <main className={`flex items-center justify-center p-4 bg-[var(--t-bg)] ${hasCampanha ? 'lg:flex-1' : 'flex-1'}`}>
