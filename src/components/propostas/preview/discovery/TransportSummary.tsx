...
function FlightCard({ voo, idioma, cor, onClick }: { voo: TransporteData; idioma: IdiomaProposal; cor: string; onClick?: () => void }) {
  ...
  return (
    <div className={`rounded-xl overflow-hidden border-2 shadow-md transition-shadow ${onClick ? 'cursor-pointer hover:shadow-lg' : ''}`} style={{ borderColor: cor }} onClick={onClick}>
      {/* Header — cor primaria */}
...
export function TransportSummary({ transportes, idioma, corPrimaria }: Props) {
  ...
  const [selectedFlight, setSelectedFlight] = useState<TransporteData | null>(null);
          ...
          {voos.length > 0 && (
            <div className="mb-10">
              ...
              <div className="space-y-4">
                {voos.map((v, i) => (
                  <FlightCard key={v.id || i} voo={v} idioma={idioma} cor={cor} onClick={() => setSelectedFlight(v)} />
                ))}
              </div>
            </div>
          )}
