  ...
  return (
    <div className="space-y-8">
      <div className="bg-[var(--t-header-bg)] text-[var(--t-header-text)] p-4 rounded-lg flex flex-wrap gap-6">
        <div><span className="text-xs text-[var(--t-accent)]">Total ADT</span><div className="text-lg font-bold">{formatBRL(totals.totalAdt)}</div></div>
        <div><span className="text-xs text-[var(--t-accent)]">Total CHD</span><div className="text-lg font-bold">{formatBRL(totals.totalChd)}</div></div>
        <div><span className="text-xs text-[var(--t-accent)]">Total INF</span><div className="text-lg font-bold">{formatBRL(totals.totalInf)}</div></div>
        <div><span className="text-xs text-[var(--t-accent)]">Total Meia</span><div className="text-lg font-bold">{formatBRL(totals.totalMeia)}</div></div>
      </div>
          ...
          <div key={aIdx} className="border rounded-lg overflow-hidden">
            <div className="bg-[var(--t-header-bg)] text-[var(--t-header-text)] p-3 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <Input value={atr.nome} onChange={e => updateAtrativo(aIdx, 'nome', e.target.value)} placeholder={`Atrativo ${aIdx + 1}`} className="h-8 w-48 bg-[var(--t-input-bg)] text-[var(--t-text)] border-[var(--t-border)]" />
                <Input type="date" value={atr.data || ''} onChange={e => updateAtrativo(aIdx, 'data', e.target.value || null)} className="h-8 w-40 bg-[var(--t-input-bg)] text-[var(--t-text)] border-[var(--t-border)]" />
              </div>
