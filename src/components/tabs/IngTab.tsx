...
export function IngTab({ grupo, onChange }: Props) {
  ...
  const isSourceVisible = (aIdx: number, fIdx: number) => {
    const fonte = grupo.ing.atrativos[aIdx].fontes[fIdx];
    const hasData = TIPOS.some(t => {
      const val = fonte[`valor_${t}` as keyof typeof fonte] as number | null;
      return val !== null && val > 0;
    });
    return hasData || addedSources[aIdx]?.has(fIdx) || false;
  };

  const addSource = (aIdx: number, fIdx: number) => {
    setAddedSources(prev => {
      const set = new Set(prev[aIdx] || []);
      set.add(fIdx);
      return { ...prev, [aIdx]: set };
    });
    setPickerOpen(null);
  };
  ...
  const clearSource = (aIdx: number, fIdx: number) => {
    const ing = { ...grupo.ing, atrativos: [...grupo.ing.atrativos] };
    ing.atrativos[aIdx] = { ...ing.atrativos[aIdx], fontes: [...ing.atrativos[aIdx].fontes] };
    ing.atrativos[aIdx].fontes[fIdx] = {
      ...ing.atrativos[aIdx].fontes[fIdx],
      valor_adt: null, valor_chd: null, valor_inf: null, valor_meia: null,
    };
    onChange({ ...grupo, ing });
    setAddedSources(prev => {
      const set = new Set(prev[aIdx] || []);
  ...
  return (
    <div className="space-y-6">
      ...
      {grupo.ing.atrativos.map((atr, aIdx) => {
        ...
        const visibleIndices = atr.fontes.map((_, i) => i).filter(i => isSourceVisible(aIdx, i));
        const hiddenSources = atr.fontes.map((f, i) => ({ nome: f.nome, idx: i })).filter((_, i) => !isSourceVisible(aIdx, i));
            ...
            <div className="p-4 space-y-4">
                  ...
                  return (
                    <div key={fIdx} className={`rounded-xl border p-4 ${isBest ? 'border-[var(--t-status-success)]/30 bg-[var(--t-status-success-bg)]/30' : 'border-[var(--t-border)] bg-[var(--t-bg)]'}`}>
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <button onClick={() => clearSource(aIdx, fIdx)} className="w-7 h-7 rounded-lg flex items-center justify-center text-[var(--t-text-muted)] hover:text-[var(--t-status-danger)] hover:bg-[var(--t-status-danger-bg)] transition-colors">
              ...
              {hiddenSources.length > 0 && (
                <div className="relative" ref={pickerOpen === aIdx ? pickerRef : undefined}>
                  <button onClick={() => setPickerOpen(pickerOpen === aIdx ? null : aIdx)} className="w-full flex items-center justify-center gap-2 py-3 rounded-xl border-2 border-dashed border-[var(--t-border)] text-sm text-[var(--t-text-muted)] hover:border-[var(--t-green)] hover:text-[var(--t-green)] transition-colors">
                  ...
                  {pickerOpen === aIdx && (
                    <div className="absolute left-1/2 -translate-x-1/2 top-full mt-2 z-20 bg-[var(--t-surface)] border border-[var(--t-border)] rounded-xl p-1.5 w-56 dropdown-enter" style={{ boxShadow: 'var(--elevation-4)' }}>
                      ...
                      {hiddenSources.map(s => (
                        <button key={s.idx} onClick={() => addSource(aIdx, s.idx)} className="w-full text-left px-3 py-2 text-sm rounded-lg hover:bg-[var(--t-surface-hover)] text-[var(--t-text)] transition-colors">
                          {s.nome}
                        </button>
                      ))}
                    </div>
